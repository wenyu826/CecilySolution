...
CCecilyMemoryPool* CCecilyMemoryPool::m_instance = NULL;

void (*malloc_alloc_template::malloc_alloc_oom_handler)() = 0;

CCecilyMemoryPool::CCecilyMemoryPool(void)
{
	InitPool();
}
...
void* CCecilyMemoryPool::Allocate(size_t n)
{
	if ( n <= 0 )
	{
		return NULL;
...
void CCecilyMemoryPool::Deallocate(void *p, size_t n)
{
	if ( NULL == p )
	{
		return ;
...
void CCecilyMemoryPool::InitPool()
{
	m_startfree = NULL;
	m_endfree	= NULL;
	m_heapsize	= 0;
...
size_t CCecilyMemoryPool::RoundUp(size_t nBytes)
{
	if ( nBytes < 0 )
	{
		return 0;
...
size_t CCecilyMemoryPool::GetFreeListIndex(size_t nBytes)
{
	nBytes = RoundUp(nBytes);
	return (nBytes/POOL_ALIGN-1)>=0?(nBytes/POOL_ALIGN-1):0;
}

char* CCecilyMemoryPool::BlockAlloc(size_t n, int &nJobs)
{
	char *result = NULL;
...
void* CCecilyMemoryPool::Refill(size_t n)
{
...
void* CCecilyMemoryPool::GetBigMemory(size_t n)
{
	void* result = NULL;
...
void CCecilyMemoryPool::RecycleBigMemory(void *p, size_t n)
{
	if ( NULL == p || n <= 0 )
	{
		return ;
...
void CCecilyMemoryPool::CecilyAllBigMemory()
{
