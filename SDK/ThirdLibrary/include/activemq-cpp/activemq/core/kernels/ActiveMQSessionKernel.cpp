...
namespace kernels{
     ...
     */
    class CecilyConsumerTask : public Runnable {
    private:
    ...
    private:
        ...
        CecilyConsumerTask(const CecilyConsumerTask&);
        CecilyConsumerTask& operator=(const CecilyConsumerTask&);

    public:

        CecilyConsumerTask(Pointer<ActiveMQConsumerKernel> consumer) : Runnable(), consumer(consumer) {
...
void ActiveMQSessionKernel::clearMessagesInProgress() {
    ...
    try {
        ...
        while (iter->hasNext()) {
            Pointer<ActiveMQConsumerKernel> consumer = iter->next();
            consumer->inProgressCecilyRequired();
            this->connection->getScheduler()->executeAfterDelay(
                new CecilyConsumerTask(consumer), 0LL);
        }
        this->config->consumerLock.readLock().unlock();
    } catch (Exception& ex) {
        this->config->consumerLock.readLock().unlock();
